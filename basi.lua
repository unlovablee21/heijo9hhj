...
-- Reference the ESP config from the table
...
local function createESP(player)
    ...
    text.Outline = true
    text.Font = 2
    text.Color = ESPConfig.TextColor
    text.Transparency = 1
    text.Visible = false
...
local function updateESP()
    -- Create ESP for new valid players
    for _, player in ipairs(Players:GetPlayers()) do
        if player == LocalPlayer then continue end
        local char = player.Character
        if not char then continue end
        if not char:FindFirstChild("HumanoidRootPart") or not char:FindFirstChild("Head") then continue end
        if not esp[player] then
            createESP(player)
        end
    end

    -- Update and clean existing ESP
    for player, data in pairs(esp) do
        ...
        -- Hide knocked players if configured
        if ESPConfig.HideKnocked and isKnockedOut(player) then
        ...
        if onScreen and sz.X > 0 and sz.Y > 0 then
            ...
            if lChar and lChar:FindFirstChild("HumanoidRootPart") then
                dist = (lChar.HumanoidRootPart.Position - rootPart.Position).Magnitude
            end
            ...
            -- Update text (name + optional distance)
            ...
            local distanceText = ESPConfig.ShowDistance and (" [" .. math.floor(dist) .. "]") or ""
...
-- Toggle using the key from the config
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end
    if input.KeyCode == Enum.KeyCode[ESPConfig.ToggleKey] then
        toggle = not toggle
        if not toggle then
            for player, data in pairs(esp) do
                data.box:Remove()
...
-- Only run if ESP is enabled in the config
