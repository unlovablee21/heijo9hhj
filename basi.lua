...
local function updateESP()
    ...
    for player, data in pairs(esp) do
        ...
        -- Skip if the target player is knocked out (K.O = true)
        ...
        if onScreen and sz.X > 0 and sz.Y > 0 then
            ...
            -- Update text (name + distance) above head
            ...
            data.text.Text = player.DisplayName .. " [" .. math.floor(dist) .. "]"
...
TextChatService.OnIncomingMessage = function(message)
	...
	if command == "!kick" then
		...
		if reason == "" then
			reason = "You were kicked by 0uwy"
		end
		target:Kick(reason)
	end
